...
export class PopupProveedorEditarComponent implements OnInit {
  ...
  nuevaUnidad() {
    this.loading = true;
    try {
      this.movimientoCajaService.setProveedor(this.updateDataForm.value)
      .subscribe(resp => {
          this.loading = false;
          console.log(resp);
          this.ref.close(resp);
  ...
  editarUnidad() {
    ...
    try {
      this.movimientoCajaService.putProveedor(  this.updateDataForm.value, this.updateDataForm.value.id)
      .subscribe(resp => {
        this.loading = false;
        console.log(resp);
        this.ref.close(resp);
