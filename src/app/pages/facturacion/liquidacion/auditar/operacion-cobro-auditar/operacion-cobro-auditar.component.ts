...
@Component({
  ...
  templateUrl: './operacion-cobro-auditar.component.html',
  styleUrls: ['./operacion-cobro-auditar.component.css'],
...
export class OperacionCobroAuditarComponent implements OnInit {
    ...
    this.nivel = [
      ...
      {label: '3',value:3}
          ...
          this.cols = [
              ...
              { field: 'operacion_cobro_id', header: 'Cobro Nº',  width: '6%' },
  ...
  editarOperacionCobroDistribucion(){
    let data:any; 
    data = this.selecteditemRegistro;
    const ref = this.dialogService.open(PopupCobroDistribucionEditarComponent, {
    data,
     header: 'Editar distribucion', 
     width: '98%',
     height: '90%'
    });
    ref.onClose.subscribe((PopupCobroDistribucionEditarComponent:OperacionCobroDetalle) => {
     // this.loadRegistro();
        if (PopupOperacionCobroEditarComponent) {
          console.log(PopupOperacionCobroEditarComponent);    
        //  this.popItemOperacionCobro = PopupOperacionCobroEditarComponent;
    ...
    swal({
        ...
        if (result.value) {
          ...
          this.selecteditems.forEach(element => {            
              element['usuario_audita_id']= userData['id']; // DEBE SER MODIFICADO PARA QUE AUDITE EL USUARIO
          }); 
        ...
        .subscribe(resp => {
          if (resp[0]) {
            for(let i=0;i<resp.length;i++){
              ...
              if((resp[i]['forma_pago'] !== 'EFECTIVO')&&(resp[i]['complejidad'] !== '3')){
...
auditarRegistros(){
  ...
  try {
    console.log(JSON.stringify(this.selecteditems));
      this.miServicio.auditarpractica(this.selecteditems)    
      .subscribe(resp => {
        console.log(resp);
        if(resp != 0){
        
          this.throwAlert('success','Se auditaron con éxito '+resp+' registros','','');
          this.loadRegistro();
        }
