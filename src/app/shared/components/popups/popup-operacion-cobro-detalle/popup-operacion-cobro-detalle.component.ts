...
@Component({
  selector: 'app-popup-operacion-cobro-detalle',
  templateUrl: './popup-operacion-cobro-detalle.component.html',
  styleUrls: ['./popup-operacion-cobro-detalle.component.css'],
  providers: [MessageService,DialogService]
})
export class PopupOperacionCobroDetalleComponent implements OnInit {
...
console.log(this.config.data);
  try {
      this.miServicio.getOperacionCobroRegistrosByIdOperacionCobro(this.config.data.obra_social_id)
      .subscribe(resp => {
        console.log(resp);
        if (resp[0]) {
          this.elementos = resp;
